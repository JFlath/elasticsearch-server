...
public class TermsFacetBuilder extends AbstractFacetBuilder {
    ...
    /**
     * Should the fact run in global mode (not bounded by the search query) or
    ...
    /**
     * An additional facet filter that will further filter the documents the
    ...
    /**
     * Sets the nested path the facet will execute on. A match (root object)
     * will then cause all the nested objects matching the path to be computed
     * into the facet.
     */
    @Override
    ...
    /**
     * Define a script field that will control the terms that will be used (and
