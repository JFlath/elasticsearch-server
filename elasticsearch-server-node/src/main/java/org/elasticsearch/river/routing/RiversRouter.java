...
public class RiversRouter extends AbstractLifecycleComponent<RiversRouter> implements ClusterStateListener {
    ...
    @Inject
    public RiversRouter(Settings settings, IngestClient client, ClusterService clusterService, RiverClusterService riverClusterService) {
        super(settings);
                    ...
                    if (!currentState.routing().hasRiverByName(mappingType)) {
                        ...
                        try {
                            ...
                            if (getResponse.exists()) {
                                String riverType = XContentMapValues.nodeStringValue(sourceAsMap(getResponse).get("type"), null);
                                if (riverType == null) {
    ...
    private Map<String, Object> sourceAsMap(GetResponse getResponse) throws ElasticSearchParseException {
