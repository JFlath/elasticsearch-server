...
public class RiversService extends AbstractLifecycleComponent<RiversService> {
    ...
    private ServerIndicesAdminClient adminClient;
        ...
        public void riverClusterChanged(RiverClusterChangedEvent event) {
            ...
            for (final RiverName riverName : rivers.keySet()) {
                ...
                if (routing == null || !localNode.equals(routing.node())) {
                    ...
                    try {
                        ingestClient.prepareGet(riverIndexName, riverName.name(), "_meta").setListenerThreaded(true).execute(new ActionListener<GetResponse>() {
                            @Override
                            public void onResponse(GetResponse getResponse) {
                                if (!getResponse.exists()) {
                                    // verify the river is deleted
                                    adminClient.prepareDeleteMapping(riverIndexName).setType(riverName.name()).execute(new ActionListener<DeleteMappingResponse>() {
                                        @Override
            ...
            for (final RiverRouting routing : state.routing()) {
                ...
                ingestClient.prepareGet(riverIndexName, routing.riverName().name(), "_meta").setListenerThreaded(true).execute(new ActionListener<GetResponse>() {
                    @Override
                    public void onResponse(GetResponse getResponse) {
                        if (!rivers.containsKey(routing.riverName())) {
                            if (getResponse.exists()) {
                                // only create the river if it exists, otherwise, the indexing meta data has not been visible yet...
                                createRiver(routing.riverName(), sourceAsMap(getResponse));
                            }
                        }
                    }
    ...
    public Map<String, Object> sourceAsMap(GetResponse getResponse) throws ElasticSearchParseException {
