 ...
 *
 */
public class TransportNodesInfoAction extends TransportNodesOperationAction<NodesInfoRequest, NodesInfoResponse, TransportNodesInfoAction.NodeInfoRequest, ServerNodeInfo> {
    ...
    protected NodesInfoResponse newResponse(NodesInfoRequest nodesInfoRequest, AtomicReferenceArray responses) {
        final List<ServerNodeInfo> nodesInfos = new ArrayList<ServerNodeInfo>();
        for (int i = 0; i < responses.length(); i++) {
            Object resp = responses.get(i);
            if (resp instanceof ServerNodeInfo) {
                nodesInfos.add((ServerNodeInfo) resp);
            }
        }
        return new NodesInfoResponse(clusterName, (NodeInfo[])nodesInfos.toArray(new ServerNodeInfo[nodesInfos.size()]));
    }
    ...
    @Override
    protected ServerNodeInfo newNodeResponse() {
        return new ServerNodeInfo();
    }

    @Override
    protected ServerNodeInfo nodeOperation(NodeInfoRequest nodeRequest) throws ElasticSearchException {
