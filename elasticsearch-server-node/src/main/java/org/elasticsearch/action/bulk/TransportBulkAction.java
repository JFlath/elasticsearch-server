...
public class TransportBulkAction extends TransportAction<BulkRequest, BulkResponse> {
    ...
    @Inject
    public TransportBulkAction(Settings settings, ServerThreadPool threadPool, TransportService transportService, ClusterService clusterService,
    ...
    private void executeBulk(final BulkRequest bulkRequest, final long startTime, final ActionListener<BulkResponse> listener) {
        ...
        for (ActionRequest request : bulkRequest.requests) {
            if (request instanceof IndexRequest) {
                ...
                IndexHelper.process(indexRequest, metaData, aliasOrIndex, mappingMd, allowIdGeneration);
            } else if (request instanceof DeleteRequest) {
                DeleteRequest deleteRequest = (DeleteRequest) request;
