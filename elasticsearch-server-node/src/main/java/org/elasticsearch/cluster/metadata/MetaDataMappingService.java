                ...
                try {
                    ...
                    for (String type : sTypes) {
                        ...
                        if (!mapper.mappingSource().equals(indexMetaData.mappings().get(type).source())) {
                            ...
                            indexMetaDataBuilder.putMapping(new ServerMappingMetaData(mapper));
                        }
                    }
        ...
        clusterService.submitStateUpdateTask("update-mapping [" + index + "][" + type + "]", new ProcessedClusterStateUpdateTask() {
            ...
            public ClusterState execute(ClusterState currentState) {
                ...
                try {
                    ...
                    MetaData.Builder builder = newMetaDataBuilder().metaData(currentState.metaData());
                    builder.put(newIndexMetaDataBuilder(indexMetaData).putMapping(new ServerMappingMetaData(existingMapper)));
                    return newClusterStateBuilder().state(currentState).metaData(builder).build();
                } catch (Exception e) {
        ...
        clusterService.submitStateUpdateTask("put-mapping [" + request.mappingType + "]", new ProcessedClusterStateUpdateTask() {
            ...
            public ClusterState execute(ClusterState currentState) {
                ...
                try {
                    ...
                    final Map<String, ServerMappingMetaData> mappings = newHashMap();
                    for (Map.Entry<String, DocumentMapper> entry : newMappers.entrySet()) {
                        ...
                        if (existingMappers.containsKey(entry.getKey())) {
                            ...
                            if (existingSource.equals(updatedSource)) {
                                ...
                                mappings.put(index, new ServerMappingMetaData(existingMapper));
                                if (logger.isDebugEnabled()) {
                            ...
                            mappings.put(index, new ServerMappingMetaData(newMapper));
                    ...
                    for (String indexName : request.indices) {
                        ...
                        ServerMappingMetaData mappingMd = mappings.get(indexName);
                        if (mappingMd != null) {
