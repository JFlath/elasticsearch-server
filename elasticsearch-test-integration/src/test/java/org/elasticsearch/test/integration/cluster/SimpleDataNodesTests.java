...
public class SimpleDataNodesTests extends AbstractNodesTests {
    ...
    public void testDataNodes() throws Exception {
        ...
        try {
            client("nonData1").index(indexRequest("test").type("type1").id("1").source(source("1", "test")).timeout(timeValueSeconds(1))).actionGet();
        ...
        try {
            client("nonData2").index(indexRequest("test").type("type1").id("1").source(source("1", "test")).timeout(timeValueSeconds(1))).actionGet();
            assert false : "no allocation should happen";
        } catch (UnavailableShardsException e) {
            // all is well
        }
        ...
        IndexResponse indexResponse = client("nonData2").index(indexRequest("test").type("type1").id("1").source(source("1", "test"))).actionGet();
