 ...
 *
 */
public class NettyCachedStreamOutput {

    private static Entry newEntry() {
        NettyBytesStreamOutput bytes = new NettyBytesStreamOutput();
        HandlesStreamOutput handles = new HandlesStreamOutput(bytes);
        return new Entry(bytes, handles);
    }

    public static class Entry {
        private final NettyBytesStreamOutput bytes;
        private final HandlesStreamOutput handles;

        Entry(NettyBytesStreamOutput bytes, HandlesStreamOutput handles) {
            this.bytes = bytes;
        ...
        public NettyBytesStreamOutput bytes() {
            return bytes;
        }
        ...
        public StreamOutput bytes(BasicCompressor compressor) throws IOException {
            return compressor.streamOutput(bytes);
        }

        public StreamOutput handles(BasicCompressor compressor) throws IOException {
