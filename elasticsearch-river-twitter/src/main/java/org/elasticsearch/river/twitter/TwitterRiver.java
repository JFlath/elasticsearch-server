...
import static org.elasticsearch.client.IngestRequests.deleteRequest;
...
public class TwitterRiver extends AbstractRiverComponent implements River {
    ...
    @Inject
    public TwitterRiver(RiverName riverName, RiverSettings settings, Client client, ServerThreadPool threadPool) {
    ...
    private void reconnect() {
        ...
        try {
            ...
            threadPool.schedule(TimeValue.timeValueSeconds(10), ThreadPool.Names.CACHE, new Runnable() {
                @Override
                public void run() {
                    reconnect();
                }
    ...
    private class StatusHandler extends StatusAdapter {
        ...
        public void onStatus(Status status) {
            ...
            try {
                ...
                builder.endObject();
                currentRequest.add(indexRequest(indexName).type(typeName).id(Long.toString(status.getId())).create(true).source(builder));
                processBulkIfNeeded();
            } catch (Exception e) {
        ...
        public void onDeletionNotice(StatusDeletionNotice statusDeletionNotice) {
            if (statusDeletionNotice.getStatusId() != -1) {
                currentRequest.add(deleteRequest(indexName).type(typeName).id(Long.toString(statusDeletionNotice.getStatusId())));
                processBulkIfNeeded();
            }
        }
