...
//                .put("gateway.hdfs.uri", "hdfs://training-vm.local:8022")
    ...
    public void testHdfsGateway() throws Exception {
        ...
        node.client().index(indexRequest("test").type("type1").id("1").source(source("1", "test"))).actionGet();
        logger.info("Indexing #2");
        node.client().index(indexRequest("test").type("type1").id("2").source(source("2", "test"))).actionGet();
        ...
        node.client().index(indexRequest("test").type("type1").id("3").source(source("3", "test"))).actionGet();
